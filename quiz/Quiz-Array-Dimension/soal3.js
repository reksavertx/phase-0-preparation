...
function minDistanceBetweenGreatest(arr) {
  ...
  let objDump = {};
  let minMuncul = Infinity;
  let maxNilai = -Infinity;

  for (let i = 0; i < arr.length; i++) {
    if (objDump[arr[i]] === undefined) {
        objDump[arr[i]] = [i];
    } else {
        objDump[arr[i]].push(i);
    }
  ...
  for (let i = 0; i < objDump[maxNilai].length; i++) {
    if (objDump[maxNilai][i + 1] - objDump[maxNilai][i] < minMuncul) {
        minMuncul = objDump[maxNilai][i + 1] - objDump[maxNilai][i];
    }
  }

  if ( minMuncul === Infinity) {
    return 0;
  }
  return minMuncul;
}
