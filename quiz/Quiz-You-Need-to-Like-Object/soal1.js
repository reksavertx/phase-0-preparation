...
function ganjilGenapAsianGames(date, data) {
    ...
    for (let i = 0; i < data.length; i++) {
        data[i]["plat"] = data[i]["plat"].split(" ")
        
        let angkaPlatTerakhir = data[i]["plat"][1][ data[i]["plat"][1].length - 1 ];
        if (angkaPlatTerakhir % 2 !== date % 2 && data[i]["type"] === "Mobil") {
            output+=1
            pelaku.push(data[i])
        }
    }

    return output, pelaku
}
